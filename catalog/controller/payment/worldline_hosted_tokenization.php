<?php
class ControllerPaymentWorldlineHostedTokenization extends Controller {
	private $error = array();
	...
	public function index() {			
		...
		if ($setting['account']['api_key'][$environment] && $setting['account']['api_secret'][$environment]) {
			...
			return $this->load->view('payment/worldline/worldline_hosted_tokenization', $data);
		}
	...
	public function modal() {
		...
		$data['entry_card_token_save'] = $this->language->get('entry_card_token_save');
		...
		if ($this->customer->isLogged()) {
			$card_customer_tokens = $this->model_payment_worldline->getWorldlineCustomerTokens($this->customer->getId(), 'card');
		...
		$this->response->setOutput($this->load->view('payment/worldline/worldline_hosted_tokenization_modal', $data));
	}
}