<div id="worldline-form">
	...
	<div class="buttons">
		<div class="pull-right">
			<button type="button" id="button-confirm" class="worldline-button btn btn-primary btn-block"><?php echo $hosted_tokenization['button_title']; ?></button>
		</div>
	</div>
</div>
<style type="text/css">
...
var initWorldlineSDK = function() {
	$('#worldline-form #button-confirm').on('click', function() {	
		...
		$('#worldline-modal').load('index.php?route=payment/worldline_hosted_tokenization/modal #worldline-modal >', function() {		
			...
			if ($('#worldline-card-form').attr('hosted-tokenization-url')) {											
				...
				$('#worldline-hosted-tokenization-form .card-token-delete-button').on('click', function() {
					if (!$('#worldline-card-tokens-container').hasClass('disabled')) {
						...
						$.ajax({
							type: 'post',
							url: 'index.php?route=payment/worldline/deleteCustomerToken',
							data: {'token': token},
							dataType: 'json',
				...
				$('#worldline-hosted-tokenization-form #worldline-card-button').on('click', function() {
					tokenizer.submitTokenization().then((result) => {
						if (result.success) {
							...
							$.ajax({
								type: 'post',
								url: 'index.php?route=payment/worldline/confirm&hostedTokenizationId=' + result.hostedTokenizationId + '&cardTokenSave=' + card_token_save,
