...
class ControllerPaymentWorldline extends Controller {
	...
	public function index() {
		...
		if ($setting['account']['api_key'][$environment] && $setting['account']['api_secret'][$environment] && !$this->callback() && !$this->webhook() && !$this->cron()) {
			...
			return $this->load->view('payment/worldline/worldline_hosted_checkout', $data);
		}
	...
	public function deleteCustomerToken() {
		...
		if ($this->customer->isLogged() && isset($this->request->post['token'])) {
			$this->model_payment_worldline->deleteWorldlineCustomerToken($this->customer->getId(), 'card', $this->request->post['token']);
	...
	public function confirm() {					
		...
		$redirection_data->setReturnUrl(str_replace('&amp;', '&', $this->url->link('payment/worldline/callback', '', true)));
		...
		$card_payment_method_specific_input->setReturnUrl(str_replace('&amp;', '&', $this->url->link('payment/worldline/callback', '', true)));
		...
		if (!empty($this->request->get['hostedTokenizationId'])) {
			...
			if (!$errors) {
				...
				if (!empty($merchant_action) && ($merchant_action->getActionType() == 'REDIRECT')) {
					...
					$data['redirect'] = str_replace('&amp;', '&', $this->url->link('payment/worldline/callback', 'paymentId=' . $transaction_id, true));
				}
			}
		}	
		...
		if (empty($this->request->get['hostedTokenizationId'])) {
			...
			$redirect_payment_method_specific_input->setReturnUrl(str_replace('&amp;', '&', $this->url->link('payment/worldline/callback', '', true)));
	...
	public function cron() {
		if (!empty($this->request->get['cron_token'])) {
			...
			if (hash_equals($setting['account']['cron_token'], $this->request->get['cron_token'])) {
				...
				$waiting_capture_worldline_orders = $this->model_payment_worldline->getWaitingCaptureWorldlineOrders();
